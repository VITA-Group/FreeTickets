...
def add_parser_arguments(parser):
    ...
    parser.add_argument('--mix', default=0.1, type=float,
                        help='mix rate of gradient and random growth')
...
def train_net(args, logger_cls):
    ...
    decay = CosineDecay(args.prune_rate, len(train_loader)*100)
    mask = Masking(optimizer, prune_mode=args.prune, prune_rate_decay=decay, growth_mode=args.growth, redistribution_mode=args.redistribution, args=args)
    model_and_loss.mask = mask

    if not args.dense:
        if args.CS_ini:
            CSini_masks = CS_ini(args.density, train_loader, args=args)
            mask.add_module(model_and_loss.model, sparse_init='CS_ini', density=args.density, CSini_masks=CSini_masks, train_loader_jac=train_loader)
        else: mask.add_module(model_and_loss.model, sparse_init=args.sparse_init, density=args.density)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch < args.warmup:
            ...
            if epoch != 30:
                ...
                model_and_loss.mask.prune_rate_decay = CosineDecay(args.prune_rate, len(train_loader) * 70)

        elif epoch in minestone2: lr = 0.001
        elif epoch in minestone3: lr = 0.0001
        ...
        if epoch < 100: filename= 'checkpoint-0.pth.tar'
        elif epoch < 170: filename= 'checkpoint-1.pth.tar'
        elif epoch < 240: filename= 'checkpoint-2.pth.tar'
        elif epoch < 310: filename= 'checkpoint-3.pth.tar'
        ...
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            ...
            }, is_best, filename=filename, backup_filename=backup_filename)
...
def train(train_loader, model_and_loss, optimizer, fp16, logger, epoch, prof=False):
    ...
    for i, (input, target) in enumerate(prefetched_loader(train_loader, fp16)):
        ...
        if update_iter % args.update_frequency == 0 and not args.fix:
            model_and_loss.mask.at_end_of_epoch()
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar', backup_filename=None):
    if (not torch.distributed.is_initialized()) or torch.distributed.get_rank() == 0:
        ...
        if is_best:
            torch.save(state, os.path.join('save', filename))
