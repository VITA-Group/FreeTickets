...
def main():
    ...
    parser.add_argument('--init-prune-epoch', type=int, default=40,
                        help='Epoch to start prune')
    parser.add_argument('--final-prune-epoch', type=int, default=250,
                        help='Epoch to finish prune')
    ...
    for i in range(args.iters):
        ...
        if args.nolr_scheduler:
            ...
            lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                                milestones=[int(args.epochs/2)*args.multiplier, int(args.epochs*3/4)*args.multiplier], last_epoch=-1)
        if args.resume:
        ...
        mask = None
        if args.sparse:
            decay = CosineDecay(args.death_rate, len(train_loader)*(args.epochs_fs + args.individual_epoch))
        ...
        # recover from the initial weights
        if args.sparse_init == 'lottery_ticket':
            ini_weights = os.path.join(args.save_dir, args.ini_save)
            print_and_log("=> loading the initial checkpoint '{}'".format(ini_weights))
            model.load_state_dict(torch.load(ini_weights)['state_dict'])
        ...
        for epoch in range(1, args.epochs*args.multiplier + 1):
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                save_dir = 'results/constant/' + str(args.model) + str(args.data) + 'seed_' + str(
                    args.seed) + '_density' + str(args.density) + '_bs' + str(args.batch_size) + '_LTR'
                if not os.path.isdir(save_dir):
                    os.mkdir(save_dir)
                torch.save(model.state_dict(), save_dir + '/' + str(args.seed) + '.pt')


            if args.sparse and not args.fix:
