...
def main():
    ...
    parser.add_argument('--nolrsche', action='store_true', default=False, help='disables CUDA training')
    parser.add_argument('--no_rewire_extend', action='store_true', default=False, help='if ture, only do rewire for 250 epoochs')
    ...
    for i in range(args.iters):
        ...
        for epoch in range(1, args.epochs*args.multiplier + 1):
            save_dir = 'results' + '/' + str(args.model) + '/' + str(args.data) +  '/' + 'density_' + str(
                args.density) + '/' + 'DST' + '/' + 'seed=%d' % (args.seed)
